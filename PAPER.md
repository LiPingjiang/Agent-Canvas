...
### 10.4 Agent Ecosystem Integration

The three-layer architecture and four-step processing pipeline are not isolated designs—they map directly onto the engineering capabilities of current mainstream agent frameworks. We analyze this mapping using [Claude Code](https://code.claude.com/docs/en/overview) as a representative agent platform.

**Pipeline orchestration via Dynamic Workflows.** Claude Code's [Dynamic Workflows](https://code.claude.com/docs/en/workflows) are JavaScript scripts that move orchestration logic out of the agent conversation—scripts hold loops, branches, and intermediate results, while the agent context retains only the final answer. Our four-step pipeline (intent parsing → semantic modeling → data instantiation → visual mapping) can be implemented as a workflow script where each step's intermediate JSON is stored in script variables rather than the main context window. This directly addresses the token overhead identified in our cost-benefit analysis (Section 9): the 2-3× token increase from three-layer intermediate representations no longer consumes the main agent's context. Workflows also support adversarial review patterns—independent agents that cross-check each other's findings—which provides an agent-level implementation of our visual feedback loop.

**Quality gates via Hooks.** Claude Code's [Hooks](https://code.claude.com/docs/en/hooks) system executes shell commands at lifecycle points like `PreToolUse`, `PostToolUse`, and `Stop`, with exit code 2 blocking agent completion and returning feedback. Our three-layer feedback system (semantic review / data review / visual review) maps to Stop hooks: when the agent claims completion, hook scripts verify semantic coverage (are all intent dimensions represented?), data completeness (are DataItems valid?), and visual quality (does the screenshot pass multimodal review?). Any layer failure blocks completion with corrective feedback. Claude Code's 8-retry limit before forced completion aligns with our "maximum N iterations" design.

**Cost optimization via model routing.** Claude Code's [subagent](https://code.claude.com/docs/en/sub-agents) system supports per-subagent model assignment—Haiku for exploration (cheap, fast), Sonnet for implementation (expensive, capable). Our pipeline steps have different capability requirements: intent parsing is a classification task suitable for Haiku, semantic modeling requires deep understanding (Sonnet/Opus), data instantiation is structured extraction (Haiku), and visual mapping requires aesthetic judgment (Sonnet). Simple cases (30% fast path) can use Haiku throughout; complex cases (70% full pipeline) upgrade at the semantic modeling and visual mapping steps.

**Data access via MCP.** The [Model Context Protocol](https://code.claude.com/docs/en/mcp) provides standardized connections to external data sources. Our Data Layer's DataItem values can be populated through MCP: database connectors (PostgreSQL/MySQL), API connectors (REST/GraphQL), document connectors (Google Drive/Notion), and monitoring connectors (Sentry/Datadog). Semantic layer entities and attributes naturally map to MCP tool parameters, making Agent Canvas a complete system that connects to real data sources rather than just a design framework.

**Preference persistence via CLAUDE.md.** Claude Code's [memory system](https://code.claude.com/docs/en/memory) persists instructions across sessions. Intent parsing dimensions like audience (who) and display context (where) are stable user preferences that need not be re-parsed each time. A CLAUDE.md file can encode visualization preferences (audience type, display device, color theme, information density), and Auto Memory can learn recurring patterns from user corrections.

**Parallel generation via Agent Teams.** [Agent Teams](https://code.claude.com/docs/en/agent-teams) enable multiple agent instances to collaborate—one lead coordinates while teammates work independently and communicate directly. For Level 3 composition (multi-region dashboards), different teammates can each handle one region's semantic-to-visual mapping pipeline, with the lead performing spatial layout and synthesis. Teammates can cross-validate each other's data (e.g., checking that trend data is consistent with KPI data) through `TeammateIdle` and `TaskCompleted` hooks.

**Engineering validation via Plan Mode.** Claude Code's [Plan Mode](https://code.claude.com/docs/en/permission-modes) separates exploration from execution—agents research and propose before modifying. This validates our core principle of "understanding before rendering." Semantic Layer modeling can serve as a plan-stage product: the agent outputs semantic JSON, the user confirms "these are the meanings I want to express," and only then does the pipeline proceed to data and visual layers. This provides a natural human-in-the-loop checkpoint for the three-layer architecture.

### 10.5 Harness Engineering Methodology

Beyond mapping to specific platform features, recent industry research reveals deeper methodological principles that validate and extend our framework design. We synthesize findings from three sources: the [Claude Blog](https://claude.com/blog), [LangChain Blog](https://www.langchain.com/blog), and [NVIDIA Technical Blog](https://developer.nvidia.com/blog).

**Harness design principles.** Claude's ["Agent Harness Design: 3 Patterns"](https://claude.com/blog/agent-harness-design-3-patterns-for-harnessing-claudes-intelligence) identifies three patterns: (1) lean on the model, not the harness—harnesses encode assumptions about what models cannot do, but these grow stale as models improve; (2) strip the harness down—let agents orchestrate their own actions, manage their own context, and persist their own memory; (3) set boundaries carefully—use declarative tools for UX/security boundaries and maximize cache hits. Agent Canvas embodies all three: the three-layer architecture avoids hardcoding visual decisions (pattern 1), the Canvas Schema is an agent-authored product not a template fill (pattern 2), and the predefined component library with Zod validation defines declarative boundaries (pattern 3). The implication for future work is that as models improve at spatial reasoning, the deterministic rendering engine's constraints can be progressively relaxed.

**Loop engineering.** Claude's ["Loop Engineering"](https://claude.com/blog/getting-started-with-loops) categorizes agent loops by trigger (user prompt, goal, time, event) and stop condition. Our visual feedback loop is a goal-based loop—stop when visual review passes or maximum iterations are reached. A key insight is that deterministic stop criteria (e.g., "Lighthouse score ≥ 90") outperform subjective judgments ("looks good"). This suggests our visual review should adopt quantifiable standards: overlap area < 5%, alignment deviation < 4px, color contrast ≥ WCAG AA (4.5:1). The article also notes that "running a script is cheaper than reasoning through the steps," validating our deterministic rendering engine design.

**Eval-driven improvement.** LangChain's harness engineering work demonstrates that [tuning the harness alone (without changing models) improved Terminal Bench 2.0 scores by 13.7 points](https://www.langchain.com/blog/improving-deep-agents-with-harness-engineering), and that [harness tuning took Nemotron 3 Ultra from 0.80 to 0.86, nearly matching Opus 4.8's 0.87 at 10× lower cost](https://www.langchain.com/blog/tuning-the-harness-not-the-model-a-nemotron-3-ultra-playbook). The core principle—"evals are the training data for harness work"—means that every agent failure should become an eval case, and every fix should increase eval coverage. LangChain proposes a "sandwich" improvement pattern: Harness Engineering → Fine-Tuning → Harness Engineering. For Agent Canvas, this means: (1) first optimize the three-layer architecture and component library, (2) if a capability ceiling is reached, fine-tune a visualization-specific model using collected generation traces, (3) re-optimize the harness for the fine-tuned model. [LangSmith Engine](https://www.langchain.com/blog/introducing-langsmith-engine) automates this cycle by clustering production traces into named issues, diagnosing root causes, and proposing fixes plus evaluators—Agent Canvas should adopt similar trace mining for continuous improvement. An important limitation: "harness tuning has a ceiling—it cannot add what is not in the weights," meaning the harness cannot compensate for fundamental model deficits in spatial reasoning.

**Layered context engineering.** Claude's ["Steering Claude Code"](https://claude.com/blog/steering-claude-code-skills-hooks-rules-subagents-and-more) defines seven instruction delivery methods with different context costs and compaction behaviors. The key insight is that context cost should be minimized: CLAUDE.md (always loaded, high cost) should stay under 200 lines, while Skills (loaded on demand, low cost) and Subagents (zero main-context cost) handle procedural workflows. LangChain's Deep Agents research adds that "the purpose of the harness engineer: prepare and deliver context so agents can autonomously complete work," and identifies three middleware patterns: PreCompletionChecklistMiddleware (force verification before exit), LocalContextMiddleware (inject environment context at startup), and LoopDetectionMiddleware (break doom loops). Agent Canvas maps these to: Stop hooks for visual review, semantic-layer injection of available component catalogs, and maximum-iteration limits for correction loops. A critical failure mode identified: agents do not naturally enter build-verify loops—they tend to "write, re-read, declare done" rather than "write, test, compare against spec, fix." This explains why existing agent visualization approaches are predominantly "blind generation" and validates our visual feedback loop as an engineering patch for this behavior.

**Model and effort selection.** Claude's ["Choosing a model and effort level"](https://claude.com/blog/claude-model-and-effort-level-in-claude-code) distinguishes two orthogonal dimensions: model selection (which frozen weights, i.e., capability ceiling) and effort level (how much work—files read, tools used, verification depth). The diagnostic heuristic: if the agent had all context and genuinely tried but was wrong, the problem is too hard (use a larger model); if it skipped files or did not verify, it did not try hard enough (use higher effort). LangChain's experiments found a "reasoning sandwich" (xhigh-high-xhigh) optimal—maximum reasoning for planning and verification, moderate for implementation. For Agent Canvas, this maps to: xhigh reasoning for semantic modeling (core decisions), high for intent parsing and visual mapping, medium for data instantiation, and xhigh for visual review. Different models also require different harness configurations—Codex and Claude have different prompting guides—so Agent Canvas should maintain model-specific prompt templates.

**Cost observability.** LangChain reports that [Uber exhausted a full-year AI budget in 4 months, Microsoft cancelled Claude Code licenses, and Salesforce faces a $300M bill](https://www.langchain.com/blog/fix-your-coding-agent-bill)—cost management is now critical. The three-layer architecture provides natural cost tracking granularity: each pipeline stage's token consumption is independently measurable. [NVIDIA's agentic inference research](https://developer.nvidia.com/blog/full-stack-optimizations-for-agentic-inference-with-nvidia-dynamo/) provides optimization data: Claude Code achieves 85-97% cache hit rates per worker, with 11.7× read/write ratio (write-once-read-many pattern). Agent Teams across 4 Opus teammates achieve 97.2% aggregate cache hit. NVIDIA's "agent hints" mechanism (priority, output sequence length, speculative prefill) allows harnesses to communicate scheduling context to inference infrastructure—Agent Canvas's Dynamic Workflows design (storing intermediate JSON in script variables) naturally maximizes cache reuse.

**Large-scale parallel generation.** Claude's ["Large-Scale Code Migrations"](https://claude.com/blog/ai-code-migration) demonstrates the endpoint of agent parallelization: Bun's million-line Zig-to-Rust port consumed 5.9B tokens (~$165K) with 100% test suite passing. The core insight—"you fix the process (loop) that produced the code, not the code itself"—means that visual quality issues should be addressed by improving the generation pipeline, not patching individual outputs. Five conditions make a task suitable for parallel agent generation: (1) work is parallelizable, (2) context is clear and comprehensive, (3) there is a built-in referee (test suite/visual review), (4) the failure queue writes itself, (5) consistency and edge-case handling are required. Multi-region dashboard generation satisfies all five, making Agent Canvas a natural fit for large-scale parallel generation via Dynamic Workflows orchestrating multiple agents across regions.

**Execution environment and sandboxing.** LangChain's ["Agents need their own computer"](https://www.langchain.com/blog/agents-need-their-own-computer) argues that agents need a real execution environment—not just a context window—to verify their output. Agent Canvas's visual feedback loop (render → screenshot → review → correct) requires exactly this: a sandboxed rendering engine, screenshot service, and review model with proper isolation, resource limits, observability, and fast iteration. The snapshot-and-fork pattern is particularly valuable for visual correction: fork multiple correction strategies from a single rendered state, execute in parallel, and select the best result. A critical security insight is that prompt injection remains a risk even in sandboxes—multimodal LLM reviewing a screenshot could be influenced by malicious text embedded in the visualization. Mitigation requires structured review output (Zod-validated) and intermediate filtering of correction suggestions.

**Programmatic orchestration patterns.** LangChain's ["Dynamic Subagents"](https://www.langchain.com/blog/introducing-dynamic-subagents-in-deep-agents) and ["RLMs in Deep Agents"](https://www.langchain.com/blog/how-to-use-rlms-in-deep-agents) define six orchestration patterns directly applicable to Agent Canvas: classify-and-act (route mixed requirements to specialized agents by type), fanout-and-synthesize (parallel region generation with lead synthesis), adversarial verification (independent review agent confirms visual quality), generate-and-filter (produce multiple visual schemas, score, keep best), tournament (pairwise comparison for aesthetic judgments), and loop-until-done (exhaustive semantic-to-visual coverage checking). The RLM paper's principle of "deterministic coverage"—coverage guaranteed by code loops, not model judgment—suggests that Agent Canvas's completeness checks (all semantic elements have DataItems, all DataItems have Marks, all semantic relations have visual Relations) should be enforced programmatically. OOLONG benchmark data shows RLM-style orchestration gains increase with context length (0.58→0.67 at 64K, 0.44→0.79 at 128K), making it especially relevant for Agent Canvas's large multi-region dashboards.

**Prompt caching strategy.** LangChain's ["Prompt Caching with Deep Agents"](https://www.langchain.com/blog/deep-agents-prompt-caching) reports 49-80% token cost reduction across three providers (Claude -77%, GPT -80%, Gemini -49%). The three-layer architecture's static prefixes (semantic element catalog, component API docs, mapping rules) are natural cache candidates. The prompt should be structured as: [cache breakpoint A] system instructions + component catalog + mapping rules (session-invariant) → [cache breakpoint B] user preferences + history (low-frequency change) → [no cache] current request + intermediate JSON (high-frequency change). The visual correction loop's multi-turn nature amplifies caching benefits—"caching pays off more the longer a conversation runs." Cache degradation should be minimized by setting independent breakpoints per pipeline layer, so a change in one layer's guidance does not bust the cache for other layers.

**Managed agent infrastructure.** Claude's ["Managed Agents"](https://claude.com/blog/claude-managed-agents) provides a reference architecture for Agent Canvas production deployment: sandboxed execution, long-running sessions with state persistence, multi-agent coordination, and scoped governance. Internal testing showed up to 10-point improvement on structured file generation tasks, with largest gains on hardest problems—consistent with Agent Canvas's finding that complex cases (70%) benefit most from the three-layer architecture. The ["CLAUDE.md files" guide](https://claude.com/blog/using-claude-md-files) recommends keeping configuration under 200 lines, using `/init` for automatic project analysis, and the "start simple, expand deliberately" principle—which aligns with Agent Canvas's tiered usage (30% fast path / 70% full pipeline). The ["Complete guide to building skills"](https://claude.com/blog/complete-guide-to-building-skills-for-claude) suggests encapsulating the visual review process as a Skill with on-demand loading to minimize context cost.

**Three-layer continual learning.** Harrison Chase's ["Continual learning for AI agents"](https://www.langchain.com/blog/continual-learning-for-ai-agents) defines three layers where learning occurs: Model (weight updates via SFT/GRPO), Harness (code/prompt/tool improvements), and Context (memory/skills/instructions). The key insight is that learning is not limited to the model layer—harness and context improvements are faster and cheaper than fine-tuning, and should be exhausted first. The [Meta-Harness approach](https://yoonholee.com/meta-harness/) creates a meta-loop: agents run tasks → results are evaluated → logs are stored → a coding agent reviews traces and suggests harness code changes. For Agent Canvas, this maps to a sandwich pattern: (1) Context layer—extract user preferences and visual rules from generation traces, update CLAUDE.md and Skills; (2) Harness layer—diagnose pipeline bottlenecks, optimize prompt structure and cache breakpoints, use coding agents to auto-review traces; (3) Model layer—collect good traces as SFT data, fine-tune a visualization-specific model. Learning can operate at agent, user, or organization granularity, and updates can occur offline (batch trace mining) or in-path (real-time preference recording).

**Wiki memory.** Harrison Chase's ["Wiki Memory"](https://www.langchain.com/blog/wiki-memory) and [OpenWiki Brains](https://www.langchain.com/blog/introducing-openwiki-brains-general-purpose-wiki-memory-for-agents) propose an agent-maintained, pre-computed knowledge layer that differs from RAG: rather than retrieving raw chunks at query time, a wiki pre-computes higher-level synthesis that agents can use efficiently. "For every domain there exists a knowledge base you would be well served to create." For Agent Canvas, this means maintaining a visualization wiki: pattern files (when to use radar vs. bar vs. heatmap for comparison semantics), component guides (StatCard usage, Chart configuration, common pitfalls), and design decision records. Files are the optimal substrate—"inspectable, editable, versionable, and easy for agents to read and write." The wiki should be stored as Markdown, version-controlled in Git, and actively maintained by agents. This reduces per-query token cost (pre-computed knowledge vs. raw retrieval) and enables incremental improvement (agents update the wiki as they learn new patterns).

**Model neutrality.** LangChain's ["Why Model Neutrality Matters"](https://www.langchain.com/blog/model-neutrality) argues that model neutrality is more critical than cloud neutrality was, for three reasons: (1) the rate of change is fundamentally different—labs leapfrog each other quarterly, not yearly; (2) models are selectively commoditizing—different providers lead on different capabilities (coding, multimodal, reasoning); (3) open-weight models are a credible option. The implication for Agent Canvas is threefold: the harness must be open-source (Canvas Schema is standard JSON, not tied to any model API), multi-model (each pipeline stage can use a different provider), and profile-aware (different models receive different prompt templates and tool descriptions, not a lowest-common-denominator abstraction). The [NemoClaw Blueprint](https://www.langchain.com/blog/langchain-and-nvidia-launch-the-nemoclaw-deep-agents-blueprint) validates this: "agent performance improves when the model, harness, evals, and runtime are tuned together"—Nemotron 3 Ultra with a tuned harness achieved 0.86 at $4.48, vs. 0.87 at $43.48 for the top model (10× cost difference).

**Trace-driven observability.** ["Your coding agents are a black box"](https://www.langchain.com/blog/your-coding-agents-are-a-black-box-heres-how-to-crack-them-open) and ["Agent observability needs feedback to power learning"](https://www.langchain.com/blog/agent-observability-needs-feedback-to-power-learning) establish that traces alone are insufficient—feedback signals (was the behavior useful, accepted, rejected, inefficient, or wrong?) are needed to close the learning loop. Together, traces and feedback are the raw material for improvement at all three learning layers. Agent Canvas should record a standardized trace (user request, semantic JSON, data JSON, Canvas Schema, screenshot, review feedback, correction history, token consumption, cache hit rate) paired with feedback signals (user satisfaction, semantic accuracy, schema validity, visual review pass/fail, cost within budget). LangSmith's approach of mapping different agents' traces to a unified schema is directly applicable—Agent Canvas traces should use a standardized TypeScript interface so they can be queried, compared, and mined regardless of which model or pipeline configuration produced them. ["Your harness, your memory"](https://www.langchain.com/blog/your-harness-your-memory) adds a critical warning: memory is tied to the harness, and closed harnesses behind APIs create lock-in—"without memory, your agents are easily replicable by anyone who has access to the same tools." Agent Canvas must keep its schema, traces, wiki, and memory in open formats (JSON, Markdown) under user control.

**Self-improving quality gates.** Claude's ["New in Claude Managed Agents: dreaming, outcomes, and multiagent orchestration"](https://claude.com/blog/new-in-claude-managed-agents) introduces two mechanisms directly applicable to Agent Canvas. Dreaming is a scheduled inter-session process that reviews agent traces and memory stores, extracts patterns (recurring mistakes, converged workflows, team preferences), and curates memories for self-improvement—it can run automatically or with human review. For Agent Canvas, dreaming would periodically scan generation traces to cluster failure modes (e.g., "bar chart label overlap" appearing 15 times → add a Hook rule) and extract preference patterns (e.g., 60% of users prefer dark theme → set as default). Outcomes defines a quality rubric that a separate grader agent evaluates in its own context window—untouched by the generating agent's reasoning—to avoid confirmation bias. When the output falls short, the grader pinpoints specific issues and the agent iterates. Anthropic's internal testing showed outcomes improved task success by up to 10 points, with largest gains on hardest problems (+8.4% docx, +10.1% pptx), consistent with Agent Canvas's finding that complex cases benefit most. The critical design principle—grader independence—validates our visual review layer: the multimodal review model should be a separate invocation, not the generating model reviewing itself.

**Unknowns management.** Claude's ["A field guide to Claude Fable 5: Finding your unknowns"](https://claude.com/blog/a-field-guide-to-claude-fable-finding-your-unknowns) introduces a four-quadrant cognitive framework—known knowns (what the user explicitly states), known unknowns (what the user knows they haven't figured out), unknown knowns (what's too obvious to write down but recognized when seen), and unknown unknowns (what hasn't been considered at all). The article notes that "Claude Fable is the first model where the quality of the work is bottlenecked by my ability to clarify its unknowns." For Agent Canvas, this maps to three pre-generation strategies: (1) blind spot pass—before semantic modeling, the agent proactively surfaces unknown unknowns (time range? comparison dimensions? audience?); (2) prototype divergence—for unknown knowns ("I'll know it when I see it"), generate multiple visual directions (radar, grouped bar, heatmap, score table) and let the user choose; (3) implementation notes—during visual mapping, the agent records its assumptions inline so users can correct course early. This framework suggests that Agent Canvas's semantic layer should include a "clarification" phase before data extraction, not just a linear pipeline.

**Structured knowledge format.** LangChain's ["OpenWiki 0.2 OKF support"](https://www.langchain.com/blog/openwiki-0-2-adds-okf-support) adopts Google Cloud's [OKF (Open Knowledge Format)](https://github.com/GoogleCloudPlatform/knowledge-catalog/blob/main/okf/SPEC.md) for structuring agent wiki memory. OKF adds YAML front matter (type, title, description, tags, timestamp) to each wiki file, plus two conventions: `index.md` (directory summaries) and `logs.md` (change logs). For Agent Canvas's wiki memory (Section 10.5, "Wiki memory"), OKF enables deterministic retrieval—agents can filter by tag or category without expensive agentic search (e.g., "find all pattern docs tagged `compare`"). The `logs.md` changelog enables incremental updates—agents check what changed since last run rather than rescanning the entire wiki. OKF's open-standard status provides ecosystem compatibility with community-built viewers, renderers, and linters. The `timestamp` field enables freshness tracking, prioritizing outdated entries for update. This formalizes Agent Canvas's wiki memory from ad-hoc Markdown files into a structured, queryable knowledge base.
...
The framework's bounded generation approach (predefined components only) and visual feedback loop (render → screenshot → multimodal review → correction) address the fundamental reliability and quality challenges of agent-generated visualization. The cost-benefit analysis shows that while the three-layer architecture adds overhead for simple cases, it significantly improves quality for the 70% of cases that are medium-to-high complexity—precisely the scenarios where existing approaches fail. The harness engineering methodology (Section 10.5), synthesized from 34 articles across Claude Blog, LangChain Blog, and NVIDIA Blog (56 references total), provides a practical improvement path: eval-driven iteration, layered context engineering, model/effort routing, prompt caching, programmatic orchestration, sandboxed execution, three-layer continual learning, wiki memory, model neutrality, trace-driven observability, self-improving quality gates, unknowns management, and structured knowledge formats.

Future work includes: (1) extending to 3D visualization, (2) supporting real-time streaming data, (3) conducting controlled experiments against existing approaches, (4) implementing and open-sourcing the full framework, and (5) building a visualization quality eval suite and trace mining system based on the harness engineering methodology.
...
19. **Anthropic.** "Agent Harness Design: 3 Patterns for Harnessing Claude's Intelligence." *Claude Blog*, 2026. [Link](https://claude.com/blog/agent-harness-design-3-patterns-for-harnessing-claudes-intelligence)

20. **Anthropic.** "Loop Engineering: Getting Started with Loops." *Claude Blog*, 2026. [Link](https://claude.com/blog/getting-started-with-loops)

21. **Anthropic.** "Steering Claude Code: When to Use CLAUDE.md, Skills, Hooks, and Subagents." *Claude Blog*, 2026. [Link](https://claude.com/blog/steering-claude-code-skills-hooks-rules-subagents-and-more)

22. **Anthropic.** "Choosing a Claude Model and Effort Level in Claude Code." *Claude Blog*, 2026. [Link](https://claude.com/blog/claude-model-and-effort-level-in-claude-code)

23. **Anthropic.** "How Anthropic Runs Large-Scale Code Migrations with Claude Code." *Claude Blog*, 2026. [Link](https://claude.com/blog/ai-code-migration)

24. **Trivedy, V.** "Improving Deep Agents with Harness Engineering." *LangChain Blog*, 2026. [Link](https://www.langchain.com/blog/improving-deep-agents-with-harness-engineering)

25. **Trivedy, V.** "Improving Agents is a Data Mining Problem." *LangChain Blog*, 2026. [Link](https://www.langchain.com/blog/improving-agents-is-a-data-mining-problem)

26. **LangChain.** "Tuning the Harness, Not the Model: A Nemotron 3 Ultra Playbook." *LangChain Blog*, 2026. [Link](https://www.langchain.com/blog/tuning-the-harness-not-the-model-a-nemotron-3-ultra-playbook)

27. **Tannyhill, B.** "Introducing LangSmith Engine." *LangChain Blog*, 2026. [Link](https://www.langchain.com/blog/introducing-langsmith-engine)

28. **Ru, A.** "Your Coding Agent Bill Doubled. Here's How to Fix It." *LangChain Blog*, 2026. [Link](https://www.langchain.com/blog/fix-your-coding-agent-bill)

29. **Dhanani, I. & Kosec, M.** "Full-Stack Optimizations for Agentic Inference with NVIDIA Dynamo." *NVIDIA Technical Blog*, 2026. [Link](https://developer.nvidia.com/blog/full-stack-optimizations-for-agentic-inference-with-nvidia-dynamo/)

30. **Ru, A.** "Agents Need Their Own Computer. Here's How to Give Them One Safely." *LangChain Blog*, 2026. [Link](https://www.langchain.com/blog/agents-need-their-own-computer)

31. **Runkle, S., Francis, C. & Lovell, H.** "Introducing Dynamic Subagents in Deep Agents." *LangChain Blog*, 2026. [Link](https://www.langchain.com/blog/introducing-dynamic-subagents-in-deep-agents)

32. **Runkle, S.** "How to Use RLMs in Deep Agents." *LangChain Blog*, 2026. [Link](https://www.langchain.com/blog/how-to-use-rlms-in-deep-agents)

33. **Olsen, A.** "Prompt Caching with Deep Agents." *LangChain Blog*, 2026. [Link](https://www.langchain.com/blog/deep-agents-prompt-caching)

34. **Anthropic.** "Claude Managed Agents: Get to Production 10x Faster." *Claude Blog*, 2026. [Link](https://claude.com/blog/claude-managed-agents)

35. **Anthropic.** "Using CLAUDE.md Files: Customizing Claude Code for Your Codebase." *Claude Blog*, 2025. [Link](https://claude.com/blog/using-claude-md-files)

36. **Anthropic.** "A Complete Guide to Building Skills for Claude." *Claude Blog*, 2026. [Link](https://claude.com/blog/complete-guide-to-building-skills-for-claude)

37. **Zhang, A. et al.** "Recursive Language Models." *arXiv preprint*, 2025. [Link](https://arxiv.org/abs/2512.24601)

38. **Janicki, M.** "How We Made Coding Agent Spend Predictable." *LangChain Blog*, 2026. [Link](https://www.langchain.com/blog/how-we-made-coding-agent-spend-predictable)

39. **Chase, H.** "Continual Learning for AI Agents." *LangChain Blog*, 2026. [Link](https://www.langchain.com/blog/continual-learning-for-ai-agents)

40. **Chase, H.** "Wiki Memory." *LangChain Blog*, 2026. [Link](https://www.langchain.com/blog/wiki-memory)

41. **Sproul, B.** "Introducing OpenWiki Brains, General-Purpose Wiki Memory for Agents." *LangChain Blog*, 2026. [Link](https://www.langchain.com/blog/introducing-openwiki-brains-general-purpose-wiki-memory-for-agents)

42. **Dahlke, N.** "Why Model Neutrality Matters More Than Cloud Neutrality." *LangChain Blog*, 2026. [Link](https://www.langchain.com/blog/model-neutrality)

43. **LangChain Team.** "LangChain and NVIDIA Launch NemoClaw Deep Agents Blueprint." *LangChain Blog*, 2026. [Link](https://www.langchain.com/blog/langchain-and-nvidia-launch-the-nemoclaw-deep-agents-blueprint)

44. **Harish, H.** "Your Coding Agents Are a Black Box. Here's How to Crack Them Open." *LangChain Blog*, 2026. [Link](https://www.langchain.com/blog/your-coding-agents-are-a-black-box-heres-how-to-crack-them-open)

45. **Chase, H.** "Agent Observability Needs Feedback to Power Learning." *LangChain Blog*, 2026. [Link](https://www.langchain.com/blog/agent-observability-needs-feedback-to-power-learning)

46. **Chase, H.** "Your Harness, Your Memory." *LangChain Blog*, 2026. [Link](https://www.langchain.com/blog/your-harness-your-memory)

47. **Lee, Y.** "Meta-Harness: End-to-End Optimization of Model Harnesses." 2026. [Link](https://yoonholee.com/meta-harness/)

48. **Anthropic.** "New in Claude Managed Agents: Dreaming, Outcomes, and Multiagent Orchestration." *Claude Blog*, 2026. [Link](https://claude.com/blog/new-in-claude-managed-agents)

49. **Anthropic.** "New in Claude Managed Agents: Self-Hosted Sandboxes and MCP Tunnels." *Claude Blog*, 2026. [Link](https://claude.com/blog/claude-managed-agents-updates)

50. **Anthropic.** "Built-in Memory for Claude Managed Agents." *Claude Blog*, 2026. [Link](https://claude.com/blog/claude-managed-agents-memory)

51. **Anthropic.** "Redesigning Claude Code on Desktop for Parallel Agents." *Claude Blog*, 2026. [Link](https://claude.com/blog/claude-code-desktop-redesign)

52. **Anthropic.** "A Field Guide to Claude Fable 5: Finding Your Unknowns." *Claude Blog*, 2026. [Link](https://claude.com/blog/a-field-guide-to-claude-fable-finding-your-unknowns)

53. **Sproul, B.** "OpenWiki 0.2 Is Adopting OKF Support." *LangChain Blog*, 2026. [Link](https://www.langchain.com/blog/openwiki-0-2-adds-okf-support)

54. **Sproul, B.** "Introducing OpenWiki, an Open Source Agent for Repo Documentation." *LangChain Blog*, 2026. [Link](https://www.langchain.com/blog/introducing-openwiki-an-open-source-agent-for-repo-documentation)

55. **Tangedipalli, S.** "Deep Agents Code on NemoClaw: A Governed Blueprint for Your Most Sensitive Code." *LangChain Blog*, 2026. [Link](https://www.langchain.com/blog/deep-agents-code-on-nemoclaw-a-governed-blueprint-for-your-most-sensitive-code)

56. **Google Cloud.** "OKF: Open Knowledge Format Specification." 2026. [Link](https://github.com/GoogleCloudPlatform/knowledge-catalog/blob/main/okf/SPEC.md)
