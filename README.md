...
Agent Canvas 通过四层架构解决这个问题，核心思路是**分离关注点**——把"理解要展示什么"和"怎么渲染"拆开，让 Agent 专注于语义决策，让确定性引擎负责视觉执行。
...
| [INTERMEDIATE-LAYER.md](./INTERMEDIATE-LAYER.md) | 中间层设计 — 意图解析、十种信息元素、组织规则、SIR 语义中间表示、跨元素关联层、视觉映射 |
| [COMPOSITION.md](./COMPOSITION.md) | 组合与堆叠 — 三层组合体系、七种布局模式(含关联并列)、主辅关系、组合决策树、空间分配算法 |
| [LINKING.md](./LINKING.md) | 跨元素关联层 — 六种关联类型、关联组织规则、五种视觉化手段、Linked Views 模式、位置注册表机制 |
