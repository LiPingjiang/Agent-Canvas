...
AeSlides（2025）论文精准总结了这一矛盾：**"生成过程以文本为中心，质量标准由视觉美学决定"**。
...
LaySPA（2025, arXiv:2509.16891）的研究表明，LLM 在布局设计任务中缺乏显式的空间推理能力。当被要求"把这个卡片放在图表右侧"时，LLM 无法可靠地将这种空间关系转化为正确的 CSS 布局代码。具体表现为：
...
LayoutVLM（Stanford, 2025）和 SKE-Layout（CVPR 2025）的研究进一步证实，即使是最先进的多模态模型，在物理上合理的布局生成方面仍有显著局限。

**第二层：缺乏视觉反馈闭环**

MatPlotAgent（ACL 2024 Findings）的核心贡献是证明了视觉反馈机制的价值：通过让多模态 LLM 审查渲染结果并提供反馈，可视化质量平均提升约 12-13 分（百分制）。这说明当前绝大多数 Agent 可视化方案是"盲生成"——生成代码后没有查看渲染结果的能力。
...
VisEval 和 PlotCraft（2025）的基准研究表明：当前最强 LLM 在单图表任务成功率约 60-80%，但复杂多面板场景骤降至 20-40%。

**第四层：布局约束求解能力不足**

CSS Flexbox/Grid、绝对定位、响应式设计——这些布局系统本质上是约束求解问题。LLM 不擅长在多维约束空间中找到最优解。Vercel v0 通过 RAG + AutoFix 机制来缓解这个问题（检索相似布局模式 + 自动修复生成错误），但根本问题仍未解决。
...
**A2UI（Agent-to-User Interface）协议** — Google 主导的开放标准，Agent 发送声明式 JSON 描述 UI 意图，客户端用原生组件渲染。核心理念："传输的是界面意图而非可执行代码"。支持 React、Angular、Flutter 等多平台。GitHub 已获 9.5K+ Star。

**Vercel JSON-Render** — Vercel 推出的生成式 UI 框架，开发者用 Zod schemas 定义组件目录，LLM 生成受约束的 JSON 规范，确定性渲染器负责实例化。

**Portal UX Agent**（arXiv:2511.00843） — 提出"有界生成"概念：LLM 在高层规划 UI，确定性渲染器从审核过的组件和模板中组装最终界面。

**Vercel v0** — 从"AI 搭建网页"进化为全栈 Agent，核心是 RAG + AutoFix 机制：检索相似组件模式、生成代码、自动检测和修复错误。
...
类比：与其让 Agent 写 HTML/CSS（像 Claude Artifacts），不如让 Agent 写一个 JSON schema 描述"一个包含标题、统计卡片组和趋势图表的仪表盘"。前者需要处理几百行 CSS，后者只需要几十行 JSON。
...
系统支持渲染 schema → 截图 → 多模态 LLM 审查 → 反馈修正的闭环。这是 MatPlotAgent 证明有效的机制，但当前主流方案（Artifacts、v0 等）大多缺乏。
...
这是 Agent Canvas 区别于 Claude Artifacts、Vercel v0 等方案的关键差异化能力。
...
### 7.3 与 Vercel v0 / JSON-Render 对比
...
4. Vercel v0、Claude Artifacts 等同类方案都选择 React，验证了可行性
...
**理由**：
1. MatPlotAgent 证明视觉反馈可提升约 12-13 分（百分制）
2. 这是当前主流方案（Artifacts、v0）的核心缺失
